...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    true_labels = torch.autograd.Variable(torch.ones(logits_real.shape[0])).type(dtype)
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.autograd.Variable(torch.ones(logits_fake.shape[0])).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    true_labels = torch.autograd.Variable(torch.ones(scores_real.shape[0])).type(dtype)
...
def ls_generator_loss(scores_fake):
    ...
    true_labels = torch.autograd.Variable(torch.ones(scores_fake.shape[0])).type(dtype)

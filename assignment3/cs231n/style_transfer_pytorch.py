...
def gram_matrix(features, normalize=True):
    ...
    features_trans = features.reshape((N, C, H * W))
    # bmm = batch mm
    gram = torch.bmm(features_trans, features_trans.transpose(1, 2))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    style_loss = style_loss.to("cuda:0")
    N = len(style_layers)
    for i in range(N) :
...
def tv_loss(img, tv_weight):
    ...
    l = torch.sum((img[ : , : , : , : -1] - img[ : , : , : , 1 : ]) ** 2)
    r = torch.sum((img[ : , : , : -1, : ] - img[ : , : , 1 : , : ]) ** 2)
    loss = tv_weight * (l + r)
    return loss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
